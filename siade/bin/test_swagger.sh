...
cp ./swagger/openapi-particulier.yaml ./swagger/versionned/openapi-particulier.yaml &&
  ...
  (ruby -e 'require "yaml" ; YAML.load_file("./swagger/openapi-entreprise.yaml") == YAML.load_file("./swagger/versionned/openapi-entreprise.yaml") ? exit(0) : exit(1)' || (echo "Swagger file for API Entreprise is different after generation" && exit 1)) &&
