RSpec.describe ANTS::ExtraitImmatriculationVehicule::ExtractIdentities do
  ...
  describe 'when response body contains identities' do
    ...
    it 'extracts identity data correctly' do
      ...
      expect(identity[:identite_from_ants][:annee_date_naissance]).to eq(1955)
      expect(identity[:identite_from_ants][:mois_date_naissance]).to eq(12)
      expect(identity[:identite_from_ants][:jour_date_naissance]).to eq(8)
      expect(identity[:identite_from_ants][:code_departement_naissance]).to eq('59')
