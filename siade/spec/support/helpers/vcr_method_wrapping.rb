def wrap_with_vcr(klass, method_name, cassette_proc, method_for_cassette_proc_args)
  klass.instance_eval do
    ...
    define_method("#{method_name}_with_vcr_wrap") do |*args|
      VCR.use_cassette(cassette_proc.call(send(method_for_cassette_proc_args))) do
        send("#{method_name}_without_vcr_wrap", *args)
      end
    end

    alias_method("#{method_name}_without_vcr_wrap", method_name)
