RSpec.describe API::V2::EntreprisesRestoredController, type: :controller do
  ...
  describe 'retrievers failure', vcr: { cassette_name: 'api_insee_fr/siren/active_GE' } do
    ...
    context 'API entreprise fails' do
      before do
        allow_any_instance_of(SIADE::V3::Retrievers::EntreprisesRestored).to receive(:retrieve).and_return nil
        allow_any_instance_of(SIADE::V3::Retrievers::EntreprisesRestored).to receive(:success?).and_return false
        allow_any_instance_of(SIADE::V3::Retrievers::EntreprisesRestored).to receive(:http_code).and_return 503
        allow_any_instance_of(SIADE::V3::Retrievers::EntreprisesRestored).to receive(:errors).and_return [ProviderUnknownError.new('INSEE', 'artificial error')]
      end
...
#  end
  ...
  describe 'checks json when ceased', vcr: { cassette_name: 'api_insee_fr/siren/ceased' } do
    it_behaves_like 'ENFORCED SPECS', ceased_siren, { entreprise: { etat_administratif: { value: 'C', date_cessation: 1315173600 } } }
    it_behaves_like 'ENFORCED SPECS', ceased_siren, { etablissement_siege: { etat_administratif: { value: 'F', date_fermeture: 1315173600 } } }
  end
end
