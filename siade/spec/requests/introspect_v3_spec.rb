RSpec.describe 'Introspection v3' do
  ...
  let(:datapass_base_url) { 'https://sandbox.datapass.api.gouv.fr/demandes' }
  ...
  shared_examples 'an introspection endpoint' do
    it 'describes the token bound to the request' do
      ...
      expect(data['datapass_id']).to eq(datapass_id)
      ...
      expect(data).not_to have_key('duree_validite_restante_en_secondes')
      expect(body['links']).to eq('datapass' => "#{datapass_base_url}/#{datapass_id}")
    ...
    it 'nullifies the DataPass id and link together when the access request has a blank DataPass id' do
      authorization_request.update!(external_id: '')

      get(path, headers:)

      expect(data['datapass_id']).to be_nil
      expect(body['links']).to eq('datapass' => nil)
    end
    ...
    context 'with an editor token' do
      ...
      context 'when the recipient has delegated several authorization requests' do
        ...
        it 'describes the delegation selected by delegation_id' do
          ...
          expect(response).to have_http_status(:ok)
          expect(data['datapass_id']).to eq('67890')
          expect(body['links']).to eq('datapass' => "#{datapass_base_url}/67890")
