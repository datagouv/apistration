...
RSpec.describe 'INPI: Marques', api: :entreprise, type: %i[request swagger] do
  path '/v3/inpi/unites_legales/{siren}/marques' do
    get SwaggerData.get('inpi.marques.title') do
      ...
      too_many_requests(INPI::Marques) do
        let(:siren) { valid_siren(:inpi) }
      end
      ...
      describe 'with valid token and mandatory params', :valid do
        response '200', 'Marques trouvées', vcr: { cassette_name: 'inpi/marques/with_valid_siren' } do
          ...
          schema build_rswag_response_collection(
            ...
            item_links: SwaggerData.get('inpi.marques.links')
          )
