...
RSpec.describe 'CNAF: Allocation Adulte Handicapé', api: :particulier, type: %i[request swagger] do
  path '/api/v2/allocation-adulte-handicape' do
    get SwaggerData.get('cnaf.aah.title') do
      tags(*SwaggerData.get('cnaf.aah.tags'))
      ...
      before do
        stub_cnaf_authenticate('allocation_adulte_handicape')
      end

      describe 'without a FranceConnect token' do
        before do
          stub_cnaf_valid('allocation_adulte_handicape', siret: '10000000000008')
        end
        ...
        describe 'with valid token and mandatory params' do
          response '200', 'Dossier trouvé' do
            description SwaggerData.get('cnaf.aah.description')

            schema build_rswag_response_api_particulier(
              attributes: SwaggerData.get('cnaf.aah.attributes')
            )
        ...
        describe 'server errors' do
          ...
          response '404', 'Dossier non trouvé' do
            before do
              stub_cnaf_404('allocation_adulte_handicape')
            end
      ...
      describe 'with a FranceConnect token' do
        let(:scopes) { %i[allocation_adulte_handicape openid identite_pivot] }
        ...
        describe 'with no token and valid FranceConnect token' do
          ...
          before do
            ...
            stub_cnaf_valid_with_franceconnect_data('allocation_adulte_handicape', siret: recipient)
          end
