...
RSpec.describe 'API Particulier CNAV: Allocation enfant handicapé (AEEH) with civility', api: :particulier, type: %i[request swagger] do
  path '/v3/dss/allocation_enfant_handicape/identite' do
    get "[Identité] #{SwaggerData.get('cnav.aeeh.title')}" do
      tags(*SwaggerData.get('cnav.aeeh.tags'))
      ...
      describe 'with valid token and mandatory params', :valid do
        before do
          stub_cnav_valid('allocation_enfant_handicape', siret: '13002526500013')
        end

        describe 'when the aeeh is found' do
          response '200', 'Allocation Enfant Handicapé active trouvée' do
            description SwaggerData.get('cnav.aeeh.description')

            cacheable_response(extra_description: SwaggerData.get('cnav.commons.cache_duration'))

            schema build_rswag_response(
              attributes: SwaggerData.get('cnav.aeeh.attributes')
            )
        ...
        response '502', 'Erreur du fournisseur', document: false do
          ...
          context 'Erreur inattendue' do
            ...
            build_rswag_example(ProviderUnknownError.new(CNAV::AllocationEnfantHandicape.provider_name, 'Une erreur inattendue est survenue lors de la collecte des données'), :unexpected_error)
        ...
        common_provider_errors_request(CNAV::AllocationEnfantHandicape)
        common_network_error_request(CNAV::AllocationEnfantHandicape)
      end
    end
  end
end
