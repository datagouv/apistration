...
RSpec.describe 'Nomenclature des erreurs', api: :particulier, type: %i[request swagger] do
  path '/api/errors' do
    get 'Nomenclature des codes erreurs' do
      ...
      parameter name: :operation_id,
        ...
        example: 'api_particulier_v3_cnav_prime_activite_with_civility'
      ...
      response '200', 'Nomenclature des codes erreurs' do
        ...
        run_test! do |response|
          expect(JSON.parse(response.body)).to eq(YAML.load_file(Rails.root.join('config/errors_particulier.yml'), aliases: true))
      ...
      response '404', 'Opération inconnue' do
        ...
        build_rswag_example(UnknownOperationError.new('api_entreprise_v3_insee_unites_legales'))
