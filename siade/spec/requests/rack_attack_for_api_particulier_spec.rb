RSpec.describe 'Rack::Attack config', api: :particulier do
  ...
  RSpec.shared_examples 'returns a 401 error (particulier)' do |expected_detail|
    let(:endpoint) do
      {
    ...
    it 'returns an error message' do
      ...
      expect(response_json).to have_json_error(detail: expected_detail)
    end
  end

  context 'when the Authorization header format (Bearer) is not respected' do
    it_behaves_like 'returns a 401 error (particulier)', "Votre token n'est pas valide" do
      let(:headers_params) { { 'Authorization' => 'Beer hour' } }
    end
  end

  context 'when the Authorization header is absent' do
    it_behaves_like 'returns a 401 error (particulier)', "Votre token n'est pas renseigné" do
