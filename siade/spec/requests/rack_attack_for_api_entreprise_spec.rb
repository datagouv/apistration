RSpec.describe 'Rack::Attack config', api: :entreprise do
  ...
  RSpec.shared_examples 'returns a 401 error (entreprise)' do |expected_detail|
    let(:endpoint) do
      {
  ...
  context 'when the Authorization header format (Bearer) is not respected' do
    it_behaves_like 'returns a 401 error (entreprise)', "Votre token n'est pas valide" do
      let(:headers_params) { { 'Authorization' => 'Beer hour' } }
    end
  end

  context 'when the Authorization header is absent' do
    it_behaves_like 'returns a 401 error (entreprise)', "Votre token n'est pas renseigné" do
