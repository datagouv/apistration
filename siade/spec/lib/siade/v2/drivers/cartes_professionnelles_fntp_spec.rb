RSpec.describe SIADE::V2::Drivers::CartesProfessionnellesFNTP, :self_hosted_doc, type: :provider_driver do
  ...
  context 'siren inconnu chez FNTP', vcr: { cassette_name: 'fntp_with_not_found_siren' } do
    let(:siren) { non_existent_siren }

    its(:http_code) { is_expected.to eq(404) }
  end

  context 'siren eligible au certificat FNTP', vcr: { cassette_name: 'fntp_with_valid_siren' } do
