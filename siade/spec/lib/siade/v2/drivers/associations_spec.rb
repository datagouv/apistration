RSpec.describe SIADE::V2::Drivers::Associations, type: :provider_driver do
  ...
  context "[siret_association_utilite_publique] Rna et Sirene", vcr: { cassette_name: 'rna_association/77571979202585' }  do
    ...
    its(:id)                          { is_expected.to eq ('W751227325') }
    its(:titre)                       { is_expected.to eq ('LA PRÉVENTION ROUTIERE') }
    ...
    its(:date_declaration)            { is_expected.to eq ('1955-01-01') }
    ...
    its(:adresse_siege_complement_3)  { is_expected.to eq (nil) }
    ...
    its(:adresse_siege_commune)       { is_expected.to eq ('Paris') }
    ...
    its(:groupement)                  { is_expected.to eq (nil) }
    its(:mise_a_jour)                 { is_expected.to eq ('1955-01-01') }
    ...
    describe 'documents' do
      ...
      its('documents.size')           { is_expected.to eq(3) }
      ...
      describe 'document_structure' do
        ...
        its(['url'])       { is_expected.to match(/https:\/\/siva\.jeunesse-sports\.gouv\.fr\/api-asso\/api\/documents\/PJ/) }
      end
    end
  end
end
