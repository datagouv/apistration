...
RSpec.describe APIEntrepriseDomainConstraint do
  let(:request) { instance_double(ActionDispatch::Request) }

  context 'when v3_and_more is false' do
    subject { described_class.new }

    context 'when the request host matches /entreprise\.api/' do
      before do
        allow(request).to receive(:host).and_return('entreprise.api.example.com')
      end
    ...
    context 'when the request host does not match /entreprise\.api/' do
      before do
