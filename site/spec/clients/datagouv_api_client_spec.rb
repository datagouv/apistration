...
RSpec.describe DatagouvAPIClient do
  ...
  describe '#find_dataservice' do
    subject(:find_dataservice) { client.find_dataservice('abc123') }

    context 'when the dataservice exists' do
      before do
        stub_request(:get, "#{host}/api/1/dataservices/abc123/")
          .with(headers: { 'X-API-KEY' => token })
          .to_return(status: 200, headers: { 'Content-Type' => 'application/json' }, body: { 'id' => 'abc123', 'title' => 'Test' }.to_json)
      end

      it 'returns the parsed dataservice' do
        expect(find_dataservice).to eq({ 'id' => 'abc123', 'title' => 'Test' })
      end
    end

    context 'when the dataservice does not exist' do
      before do
        stub_request(:get, "#{host}/api/1/dataservices/abc123/").to_return(status: 404, body: '{}')
      end

      it 'raises Faraday::ResourceNotFound' do
        expect { find_dataservice }.to raise_error(Faraday::ResourceNotFound)
      end
    end
  end
  ...
  describe 'network failure' do
    ...
    before do
      stub_request(:get, "#{host}/api/1/dataservices/abc123/").to_timeout
    end

    it 'raises a Faraday error' do
      expect { find_dataservice }.to raise_error(Faraday::Error)
    end
  end
end
