...
{:.fr-text--lead}
Les gestionnaires d'établissements d'accueil du jeune enfant (EAJE) qui récupèrent les données de participation familiale via API Particulier peuvent désormais obtenir, dans le même appel, **une attestation PDF vérifiable**. Elle remplace la capture d'écran qu'ils faisaient de l'espace partenaire CAF et peut être contrôlée en quelques secondes par n'importe quel agent, avec un simple navigateur.
...
## <a name="contexte"></a>Contexte : pourquoi ajouter une attestation vérifiable ?
...
Jusqu'ici, pour calculer le tarif appliqué, les gestionnaires d'EAJE consultaient les données de la famille sur l'espace partenaire de la CAF. Puis, pour conserver un justificatif en cas de contrôle, ils faisaient une capture d'écran de cet espace, qu'ils imprimaient ou archivaient.

Avec l'utilisation de [l'API Participation familiale EAJE](<%= endpoint_path(uid: 'cnav/psu') %>), les données arrivent directement dans le logiciel de gestion. Cela permet le calcul rapide, fiable et mis à jour du tarif appliqué. En revanche, il n'y avait plus la possibilité de faire une capture d'écran pour justifier d'un tarif passé. Il fallait donc un justificatif auquel le contrôleur puisse se fier **sans avoir accès au logiciel de la structure**, et qu'il puisse vérifier plusieurs années après son émission.
...
Seules les sections couvertes par l'habilitation de la structure apparaissent : sans le scope adresse, par exemple, l'attestation ne contient pas de section adresse. Les champs vides sont omis.
...
**API Particulier ne stocke ni les attestations, ni les données qu'elles contiennent.** Le lien de téléchargement du PDF comme le lien de vérification transportent leur contenu dans un jeton chiffré et authentifié (AES-256-GCM) : le serveur le déchiffre à la volée pour produire le PDF ou afficher la page de vérification. La moindre modification d'un lien le rend invalide, et sans la clé détenue par la plateforme, il est impossible de lire son contenu ou d'en fabriquer un faux.
...
- le **lien de vérification** reste valable 5 ans, mais ne contient que des données minimisées.
...
- **Le lien de vérification donne accès aux données minimisées pendant 5 ans** à quiconque le possède : il doit être traité avec le même soin que le document lui-même.
...
L'attestation reste vérifiable **5 ans** après son émission, pour couvrir les contrôles de la CAF, qui peuvent porter sur plusieurs exercices passés. Cette durée ne peut pas être prolongée : au-delà, il faut émettre une nouvelle attestation.
