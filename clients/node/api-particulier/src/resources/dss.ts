...
export class Dss {
  ...
  async allocation_rentree_scolaire(options: { version?: number; recipient?: string; delegation_id?: string } = {}) {
    const resolvedVersion = options.version ?? 3;
    const path = (() => {
      switch (resolvedVersion) {
      case 3:
  ...
  async allocation_rentree_scolaire_identite(options: { version?: number; recipient?: string; delegation_id?: string; nom_naissance: string; nom_usage?: string; prenoms: string[]; annee_date_naissance?: string; mois_date_naissance?: string; jour_date_naissance?: string; sexe_etat_civil?: string; code_cog_insee_pays_naissance: string; code_cog_insee_commune_naissance?: string; nom_commune_naissance?: string; code_cog_insee_departement_naissance?: string }) {
    const resolvedVersion = options.version ?? 3;
    const path = (() => {
      switch (resolvedVersion) {
      case 3:
  ...
  async foyer_rsa(options: { version?: number; recipient?: string; delegation_id?: string } = {}) {
    const resolvedVersion = options.version ?? 3;
    const path = (() => {
      switch (resolvedVersion) {
      case 3:
        return `/v3/dss/foyer_rsa/france_connect`;
        default:
          throw new Error(`version ${resolvedVersion} not available for /dss/foyer_rsa/france_connect; supported: [3]`);
      }
    })();
    return this.client.get(path, { params: { 'recipient': options.recipient, 'delegation_id': options.delegation_id } });
  }
